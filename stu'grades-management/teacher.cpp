...
{
	Teacher save1, save2, save3;

	//老师1
	save1.tea_num = "1111";
	save1.tea_name = "KSJ";
	save1.tea_collage = "scs";
	for (int i = 0; i < course.size(); i++) {
		if (course[i].teacher_num == save1.tea_num) {
			save1.tea_course.push_back(course[i]);
		}
	}
	teacher.push_back(save1);

	//老师2
	save2.tea_num = "2222";
	save2.tea_name = "KNJ";
	save2.tea_collage = "scs";
	for (int i = 0; i < course.size(); i++) {
		if (course[i].teacher_num == save2.tea_num) {
			save2.tea_course.push_back(course[i]);
		}
	}
	teacher.push_back(save2);

	//老师3
	save3.tea_num = "3333";
	save3.tea_name = "MYK";
	save3.tea_collage = "scs";
	for (int i = 0; i < course.size(); i++) {
		if (course[i].teacher_num == save3.tea_num) {
			save3.tea_course.push_back(course[i]);
		}
	}
	teacher.push_back(save3);
